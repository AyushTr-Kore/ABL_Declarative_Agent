...
| EmbeddedKnowledge | Small, stable, approved reference files shipped with this app | Enabled with five curated Arch ABL/debug references |
...
The package currently embeds the five focused references copied from the Arch
MCP debug fallback documentation:
...
Add approved organization-specific files later for the ABL authoring
structure, engineering SOP, or troubleshooting runbook. Keep those files
separate from the copied platform contracts so they can be reviewed and
...
4. Keep `CodeInterpreter` and `WebSearch` as separate capabilities. They serve
   ...
   baseline.
5. Validate and package the app with Microsoft 365 Agents Toolkit, then test
